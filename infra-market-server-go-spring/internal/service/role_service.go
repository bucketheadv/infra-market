...
func (s *RoleService) GetRoles(query dto.RoleQueryDto) dto.ApiData[dto.PageResult[dto.RoleDto]] {
	...
	if err != nil {
		log.Errorf(context.Background(), log.TagAppDef, "获取角色列表失败: %v\n", err)
...
func (s *RoleService) GetAllRoles() dto.ApiData[[]dto.RoleDto] {
	...
	if err != nil {
		log.Errorf(context.Background(), log.TagAppDef, "获取所有激活角色失败: %v\n", err)
...
func (s *RoleService) GetRole(id uint64) dto.ApiData[dto.RoleDto] {
	...
	if err != nil {
		log.Errorf(context.Background(), log.TagAppDef, "获取角色详情失败，角色ID: %d, 错误: %v\n", id, err)
...
func (s *RoleService) CreateRole(form dto.RoleFormDto) dto.ApiData[dto.RoleDto] {
	...
	if err != nil {
		log.Errorf(context.Background(), log.TagAppDef, "创建角色失败，角色编码: %s, 错误: %v\n", form.Code, err)
...
func (s *RoleService) UpdateRole(id uint64, form dto.RoleFormDto) dto.ApiData[dto.RoleDto] {
	role, err := s.roleRepo.FindByID(id)
	if err != nil {
		log.Errorf(context.Background(), log.TagAppDef, "更新角色失败，角色ID: %d, 错误: %v\n", id, err)
		return dto.Error[dto.RoleDto]("角色不存在", http.StatusNotFound)
	}
...
func (s *RoleService) DeleteRole(id uint64) dto.ApiData[any] {
	role, err := s.roleRepo.FindByID(id)
	if err != nil {
		log.Errorf(context.Background(), log.TagAppDef, "删除角色失败，角色ID: %d, 错误: %v\n", id, err)
...
func (s *RoleService) UpdateRoleStatus(id uint64, status string) dto.ApiData[any] {
	role, err := s.roleRepo.FindByID(id)
	if err != nil {
		log.Errorf(context.Background(), log.TagAppDef, "更新角色状态失败，角色ID: %d, 错误: %v\n", id, err)
		return dto.Error[any]("角色不存在", http.StatusNotFound)
	}
	...
	role.Status = status
	if err := s.roleRepo.Update(role); err != nil {
		log.Errorf(context.Background(), log.TagAppDef, "更新角色状态失败，角色ID: %d, 状态: %s, 错误: %v\n", id, status, err)

                    ...
                    <a-card size="small" class="field-card">
                      ...
                      <div class="field-info">
                        <div class="field-info-grid">
                          ...
                          <div class="field-info-item" v-if="field.type === 'COMPONENT'">
                            <span class="field-info-label">组件ID</span>
                            <span class="field-info-value">{{ field.componentId || '-' }}</span>
                          </div>
                          <div class="field-info-item" v-if="field.type === 'COMPONENT'">
                            <span class="field-info-label">是否数组</span>
                            <a-tag :color="(field.isArray || (field as any).array) ? 'green' : 'default'" size="small">
                              {{ (field.isArray || (field as any).array) ? '是' : '否' }}
                            </a-tag>
                          </div>
                          <div class="field-info-item" v-if="field.type === 'COMPONENT' && (field.isArray || (field as any).array)">
                            <span class="field-info-label">允许动态增删</span>
                            <a-tag :color="field.allowDynamic ? 'green' : 'default'" size="small">
                              {{ field.allowDynamic ? '是' : '否' }}
                            </a-tag>
                          </div>
                          <div class="field-info-item" v-if="field.type !== 'COMPONENT'">
                          ...
                          <div class="field-info-item field-info-item-full" v-if="field.type !== 'COMPONENT'">
    ...
    >
      ...
      >
        ...
        <a-form-item label="数据类型" name="type">
          <a-select v-model:value="fieldForm.type" placeholder="请选择数据类型" @change="handleTypeChange">
        ...
        <a-form-item v-if="fieldForm.type === 'COMPONENT'" label="活动组件" name="componentId">
          <a-select v-model:value="fieldForm.componentId" placeholder="请选择活动组件">
            <a-select-option
        ...
        <a-form-item v-if="fieldForm.type === 'COMPONENT'" label="是否数组">
          <a-switch v-model:checked="fieldForm.isArray" />
          <div style="margin-top: 4px; font-size: 12px; color: #999;">
            开启后，该组件可以重复多次（数组形式）
          </div>
        </a-form-item>
        <a-form-item v-if="fieldForm.type === 'COMPONENT' && fieldForm.isArray" label="允许动态增删">
          <a-switch v-model:checked="fieldForm.allowDynamic" />
          <div style="margin-top: 4px; font-size: 12px; color: #999;">
            开启后，在使用时可以动态添加/删除该组件
          </div>
        </a-form-item>
        <a-form-item v-if="fieldForm.type !== 'COMPONENT'" label="输入类型" name="inputType">
        ...
        <a-form-item v-if="fieldForm.type !== 'COMPONENT'" label="是否必填" name="required">
        ...
        <a-form-item v-if="fieldForm.type !== 'COMPONENT'" label="占位符" name="placeholder">
        ...
        <a-form-item v-if="fieldForm.type !== 'COMPONENT' && fieldForm.inputType && ['SELECT', 'MULTI_SELECT'].includes(fieldForm.inputType)" label="选项配置">
...
const handleTypeChange = () => {
  // 当类型切换为 COMPONENT 时，清空输入类型和默认值
  if (fieldForm.type === 'COMPONENT') {
    fieldForm.inputType = undefined
    fieldForm.defaultValue = undefined
    fieldForm.placeholder = undefined
    fieldForm.options = []
  } else {
    // 当类型切换为非 COMPONENT 时，清空组件相关字段
    fieldForm.componentId = undefined
    fieldForm.isArray = false
    fieldForm.allowDynamic = false
    if (!fieldForm.inputType) {
...
const handleSaveField = async () => {
  try {
    ...
    if (defaultValue !== undefined && defaultValue !== null) {
      ...
      } else if (fieldForm.type === 'DATE' || fieldForm.type === 'DATETIME') {
        // 日期类型：由于使用了 value-format，已经是字符串格式，直接使用
    ...
    const field: ActivityComponentField = {
      ...
      type: fieldForm.type,
      inputType: fieldForm.type === 'COMPONENT' ? undefined : fieldForm.inputType,
      required: fieldForm.type === 'COMPONENT' ? undefined : fieldForm.required,
      sort: editingFieldIndex.value !== null ? editingFieldIndex.value : form.fields.length,
      placeholder: fieldForm.type === 'COMPONENT' ? undefined : fieldForm.placeholder,
      description: fieldForm.description,
      defaultValue: fieldForm.type === 'COMPONENT' ? undefined : defaultValue,
      options: fieldForm.type === 'COMPONENT' ? undefined : fieldForm.options?.filter(opt => opt.value) || [],
      componentId: fieldForm.type === 'COMPONENT' ? fieldForm.componentId : undefined,
      isArray: fieldForm.type === 'COMPONENT' ? fieldForm.isArray : undefined,
      allowDynamic: fieldForm.type === 'COMPONENT' && fieldForm.isArray ? fieldForm.allowDynamic : undefined
    }

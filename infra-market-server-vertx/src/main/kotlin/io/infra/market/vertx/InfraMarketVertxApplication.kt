 ...
 * @since 1.0.0
 */
class InfraMarketVertxApplication : AbstractVerticle() {
    ...
    override fun start(startPromise: Promise<Void>) {
        ...
        try {
            ...
            val router = MainRouter.create(vertx, dbPool, redis, redisAPI)
            ...
            vertx.createHttpServer()
                ...
                .onFailure { error ->
                    logger.error("Infra Market Vert.x 服务器启动失败", error)
    ...
    private fun loadConfig(): JsonObject {
        ...
        // 从配置文件加载
        val fileConfig = ConfigLoader.loadConfig(vertxConfig ?: JsonObject())
        
        // 如果 Vert.x 配置存在，则合并（Vert.x 配置优先级更高）
        return if (vertxConfig != null && vertxConfig.size() > 0) {
